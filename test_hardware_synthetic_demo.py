#!/usr/bin/env python3
"""
Simple demonstration of hardware metrics + synthetic stress testing.
...
class HardwareMonitor:
    """Monitor for hardware metrics (real or simulated)."""
    ...
    def __init__(self):
        ...
        if HARDWARE_AVAILABLE:
            try:
                ...
                # Use power instead of energy for compatibility
...
class HardwareSyntheticDemo:
    """Demonstration of hardware metrics + synthetic stress testing."""
    ...
    def run_demo(self, num_batches=50):
        """Run the demonstration."""
        print(f"\n=== Hardware + Synthetic Stress Demo ===")
        ...
        for batch_idx in range(num_batches):
            ...
            if current_scenario is None or scenario_batch_count >= 20:
                scenario_idx = (batch_idx // 20) % len(self.stress_scenarios)
        ...
        avg_adaptation = np.mean([
            self._calculate_adaptation_score(scenario, expert_usage_history)
            for scenario in self.stress_scenarios
        ])
...
def main():
    """Run the hardware + synthetic stress demonstration."""
    print("Hardware + Synthetic Stress TTT Demo")
    print("This demo shows how the router responds to controlled synthetic stress scenarios")
    print("while using real hardware metrics when available.")
    
    # Run demo
    demo = HardwareSyntheticDemo(num_experts=16, d_model=768)
    
    try:
        results = demo.run_demo(num_batches=60)
        
        # Save results
        with open('hardware_synthetic_demo_results.json', 'w') as f:
            json.dump(results, f, indent=2, default=lambda x: float(x) if isinstance(x, torch.Tensor) else x)
        
        print(f"\nResults saved to hardware_synthetic_demo_results.json")
