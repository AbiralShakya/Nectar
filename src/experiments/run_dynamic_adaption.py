...
def run_experiment_phase(args, moe_config: MoEConfig, model: MoETransformerBlock, dataloader_manager: DataLoaderManager,
    ...
    """
    Runs a single phase of the experiment for dynamic adaptation strategies.
    """
    ...
    # Estimate base latency for TTHA penalties (essential for this script)
            ...
            with torch.profiler.profile(
                ...
                for epoch in range(epoch_offset, epoch_offset + args.epochs):
                    for batch_idx, (x, y) in enumerate(dataloader):
                        ...
                        gpu_stats = gpu_monitor.get_current_stats() # Get latest stats after TTHA update
                        log_data = {
                            ...
                            "ttha_total_loss": ttha_loss_components.get("total_loss",0), # Added TTHA losses
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NECTAR MoE Routing Experiment - Dynamic Adaptation")
    ...
    parser.add_argument("--profile_dir", type=str, default="experiment_logs/tb_logs_dynamic", help="Directory for TensorBoard profiles.")
    parser.add_argument("--log_file", type=str, default="experiment_logs/metrics_dynamic.csv", help="CSV file to log metrics.")
    ...
    parser.add_argument("--ttha_latency_penalty_weight", type=float, default=0.1, # This is now objective_weights['performance']
                        help="Placeholder, actual weight is from objective_weights in router.")
    ...
    moe_config = MoEConfig(
        ...
        top_k=args.top_k,
        # Add other config params if they are arguments to the parser
    ...
    print("\n--- Starting Phase 2: Dynamic Hardware-Aware Adaptation ---")

    model_dynamic = MoETransformerBlock(
        moe_config, kernel_cost_model, gpu_monitor
    ).to(device)

    # Note: TTHA target parameters are passed directly to router.update_ttha call
    # The default for base_latency_for_penalty will be estimated internally if not set.
    run_experiment_phase(args, moe_config, model_dynamic, dataloader_manager, gpu_monitor, metrics_logger,
                         [RoutingStrategy.KERNEL_AWARE_TTHA], # Only run TTHA strategy here
                         kernel_cost_model, epoch_offset=0)

    print("\n--- Phase 2 Complete. Data collected in metrics_dynamic.csv ---")
