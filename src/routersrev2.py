...
class AdaptiveRouter(nn.Module):
    ...
    def _initialize_expert_profiles(self) -> List[ExpertProfile]:
        """Initialize detailed expert profiles"""
        profiles = []
        for expert_id in range(self.num_experts):
            # In production, these would be learned from actual expert behavior
            specialization = np.random.uniform(0.6, 0.9)
            thermal_sensitivity = np.random.uniform(0.8, 1.2)
            
            profile = ExpertProfile(
                expert_id=expert_id,
                operations={
                    'linear_fc1': {'energy': 0.5, 'latency': 0.1, 'flops': 1e9, 'memory': 1024},
                    'relu': {'energy': 0.05, 'latency': 0.01, 'flops': 1e6, 'memory': 128},
                    'linear_fc2': {'energy': 0.3, 'latency': 0.08, 'flops': 8e8, 'memory': 768}
                },
                specialization_score=specialization,
                load_balancing_weight=1.0,
                thermal_sensitivity=thermal_sensitivity,
                memory_footprint=int(1024 * (1 + np.random.uniform(-0.2, 0.2))),
                cache_efficiency=np.random.uniform(0.7, 0.95)
            )
